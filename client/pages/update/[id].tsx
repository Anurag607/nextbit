...
const CustomEditor = dynamic(
  () => import("../../src/components/customEditor"),
  {
    ssr: false,
  }
);
...
const Page: NextPage<{
  ...
  content: string;
  post: postType;
}> = ({ userDetails, introContent, content, post }) => {
  const router = useRouter();

  const { id } = router.query;

  const [heroContent, setHeroContent] = React.useState({
    title: JSON.parse(introContent).hasOwnProperty("title")
      ? JSON.parse(introContent).title
      : post.title,
    desc: JSON.parse(introContent).hasOwnProperty("desc")
      ? JSON.parse(introContent).desc
      : post.desc,
    introImage: JSON.parse(introContent).hasOwnProperty("introImage")
      ? JSON.parse(introContent).introImage
      : post.introImage,
    bgImage: JSON.parse(introContent).hasOwnProperty("bgImage")
      ? JSON.parse(introContent).bgImage
      : post.bgImage,
  });

  const [blogData, SetblogData] = React.useState(post);
  const [editorContent, Setcontent] = React.useState(
    content === "" ? "" : JSON.parse(content)
  );
  const [auth, setAuth] = React.useState(JSON.parse(userDetails));

  const [bgImageURL, SetbgImageURL] = React.useState(
    typeof Cookie.get("bgImageURL") !== "undefined"
      ? Cookie.get("bgImageURL")
      : heroContent.bgImage
  );
  const [introImageURL, SetintroImageURL] = React.useState(
    typeof Cookie.get("introImageURL") !== "undefined"
      ? Cookie.get("introImageURL")
      : heroContent.introImage
  );
  ...
  const updatePost = async () => {
    ...
    let data = {
      ...
      content:
        typeof localStorage.getItem("content") === "undefined"
          ? post.content
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_LOCALHOST_SERVER}/api/posts/updatePost/${id}`,
      {
        method: "PUT",
        mode: "cors",
    ...
    <div className={styles.blogFormWrapper}>
      ...
      <div className={styles.content}>
        <div className={styles.heading}>
          ...
          <p>
            *the information you enter in the below form will be used to display
            your blog on the home page and for hero section of your blog page.
        ...
        >
          ...
          <div className={`${styles.inputField} ${styles.introImage}`}>
            <div className={`${styles.inputDiv}`}>
              <label htmlFor={"introImage"}>
                {"Introductory Image/Illustration : "}
              </label>
              <span>
                <input
                  type="file"
                ...
                <a
                  ...
                  rel="noreferrer"
                >
                  *For referrence you can search on <b>freepik.com</b>
                </a>
              </span>
            </div>
            <Loader />
            <div
              className={`${styles.uploadedImg} uploadedIntroImg`}
          ...
          <div className={`${styles.inputField} ${styles.bgImage}`}>
            <div className={`${styles.inputDiv}`}>
              <label htmlFor={"bgImage"}>
                {"Background Image for Hero Section of the Blog : "}
              </label>
              <span>
                <input
                  type="file"
                ...
                <a
                  ...
                  rel="noreferrer"
                >
                  *For referrence you can search on <b>unsplash.com</b>
                </a>
              </span>
            </div>
            <Loader />
            <div
              className={`${styles.uploadedImg} uploadedBgImg`}
          ...
          <div className={styles.main}>
            ...
            <input
              type="submit"
              className={`${styles.save_btn} save_btn`}
              value="Save Changes"
            />
          </div>
        </form>
