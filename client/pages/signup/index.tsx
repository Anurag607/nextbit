...
type signupTemplate = { cnfrmpass: string, email: string, username: string, password: string, user_type: string }
        ...
        if(signupdet.username.length == 0) {
            ...
            .then(resMessage => {
                if(status === 200) {
                    ...
                    styling.warning.current!.style.display = 'block'
                    styling.warning.current!.innerHTML = (status === 400) ? `User already exists` : `Failed To Create Your Account! Try Again in a Few Minutes.`
                    styling.username.current!.style.border = '0.05rem solid red'
                    styling.heading.current!.style.marginBottom = '0.5rem'
                }
            })
        }
    }, [signupdet]) // eslint-disable-line
            ...
            <section>
                <form onSubmit={HandleSubmit}>
                    ...
                    <span className={styles.tologin}>
                        ...
                        <Link href='/login' as='/login' passHref><a className={styles.loginLinks}>Login Here</a></Link>
                    </span>
                </form>
            </section>
        </main>
