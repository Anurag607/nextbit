...
function Home({ posts, userDetails }) {
  ...
  React.useEffect(() => {
    ...
    let currentPostIndex =
      Cookie.get("currentIndex") === undefined
        ? 0
        : parseInt(Cookie.get("currentIndex"));
    if (currentPostIndex > posts.length - 1) {
      Cookie.set("currentIndex", `${posts.length - 1}`);
      currentPostIndex = posts.length - 1;
    }
    const page = document.querySelectorAll(".menubtn");
    if (page[currentPostIndex] !== undefined)
      (page[currentPostIndex] as HTMLSpanElement).scrollIntoView({
        behavior: "smooth",
      });
    setArticle({
  ...
  React.useEffect(() => {
    let currentPostIndex =
      Cookie.get("currentIndex") === undefined ? 0 : Cookie.get("currentIndex");
    const page = document.querySelectorAll(".menubtn");
    if (page[currentPostIndex] !== undefined)
      (page[currentPostIndex] as HTMLSpanElement).scrollIntoView({
        behavior: "smooth",
  ...
  const Pagegen = (props: { number: string; id: number }) => {
    return (
      <span onClick={Handlechange} className="menubtn" data-index={props.id}>
        {props.number}
      </span>
    );
  };
  ...
  const Page = () => {
    return (
      <section className={styles.sidebar}>
        {(posts !== undefined && posts !== null ? posts : []).map(
          (el: Object, i: number) => {
        ...
        <div className={styles.heroSection}>
          <section className={styles.heroContent}>
            ...
            <Link
              href={`/${
                userDetails[0] !== "{" &&
                JSON.parse(userDetails).username !== undefined &&
                JSON.parse(userDetails).username.length > 0
                  ? "blogForm"
                  : "login"
              }`}
            >
          ...
          <section className={styles.postBody}>
            <section className={styles.post}>
              ...
              <div
                ...
                style={{
                  fontSize: article.title.length > 70 ? "1.5rem" : "2.25rem",
                }}
              >
                {" "}
                {article.title}{" "}
              ...
              <div
                style={{ backgroundImage: `url(${article.introImage})` }}
...
export async function getServerSideProps({ params, req, res }) {
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_LOCALHOST_SERVER}/api/posts/getAll`,
    {
      method: "GET",
      mode: "cors",
